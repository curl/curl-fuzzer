...
/// Expose generated bytes through curl's real filename-backed MIME callbacks.
/// The owner is retained beside the MIME tree because curl opens and reads the
/// path lazily during transfer, including after a redirect-triggered rewind.
...
curl_mime* BuildMimePost(CURL* easy, const curl::fuzzer::proto::MimePost& source,
  ...
  for (std::size_t i = 0; i < count && remaining_parts != 0; ++i) {
    ...
    // curl_mime_filedata sets the remote filename to the local basename as a
    // side effect. Apply metadata last so a scenario filename overrides the
    // anonymous /proc/self/fd basename on the wire.
    ApplyPartMetadata(part, proto_part, stats);
  }
  return mime;
}
