...
/// in-callback behaviour. The write callback itself runs under
/// CURL_CBAPI_START, which puts it on multi->callstack, so non-reentrant multi
/// entrypoints are refused even on the replay that curl_easy_pause() triggers.
/// The table below still holds only calls that stay safe if the guard admits
/// them.
...
/// The write callback runs under CURL_CBAPI_START, so multi->callstack.count is
/// already nonzero while the probes fire and Curl_mapi_enter refuses these
/// non-reentrant multi entries. They stay out regardless, so the harness never
/// relies on that refusal: if the guard ever admitted one, cleanup would free
/// the multi that DriveScenario keeps driving (use-after-free plus a second
/// cleanup in multi.reset()), and remove_handle would run multi_done() and
/// detach the still-driven easy, leaving the drive loop on a detached transfer
/// so it never reports CURLE_OK. The probes below neither release the multi nor
/// detach the driven easy.
