...
void TestGeneratedMimeFileUsesBoundedAnonymousStorage() {
  ...
  curl::fuzzer::proto::Scenario scenario;
  auto *file_part = scenario.mutable_mime_post()->add_parts();
  file_part->set_filename("boundary.bin");
  auto *file_data = file_part->mutable_file_data();
  file_data->set_pattern("F");
  file_data->set_repeat_count(std::numeric_limits<std::uint32_t>::max());

  // A later memory-backed source proves both representations debit the same
  // aggregate runtime budget rather than each materializing 64 KiB.
  auto *memory_data =
      scenario.mutable_mime_post()->add_parts()->mutable_generated_data();
  memory_data->set_pattern("M");
  memory_data->set_repeat_count(std::numeric_limits<std::uint32_t>::max());

  {
    ...
    Expect(request_data.stats().generated_mime_bytes ==
               proto_fuzzer::scenario_limits::kMaxGeneratedMimeDataBytes,
           "MIME file and memory sources exceeded their shared runtime budget");
  }
