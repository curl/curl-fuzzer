...
void TestFtpQuoteListsUseBoundedPointerState() {
  CURL *easy = curl_easy_init();
  Expect(easy != nullptr, "curl_easy_init failed for FTP quote lists");
  ...
  for (std::size_t index = 0;
    ...
    scenario.add_ftp_prequote(std::string(
        proto_fuzzer::scenario_limits::kMaxFtpQuoteCommandBytes + 7, 'p'));
    scenario.add_ftp_postquote(std::string(
        proto_fuzzer::scenario_limits::kMaxFtpQuoteCommandBytes + 7, 'q'));
  }

  {
    // Construct directly to prove the runtime protects compatibility inputs
    // that did not pass through the fixed-target protobuf postprocessor.
    proto_fuzzer::ScenarioRequestData request_data(easy, scenario);
    Expect(request_data.stats().ftp_prequote_commands ==
               proto_fuzzer::scenario_limits::kMaxFtpQuoteCommands,
           "FTP prequote runtime exceeded its command budget");
    Expect(request_data.stats().ftp_postquote_commands ==
               proto_fuzzer::scenario_limits::kMaxFtpQuoteCommands,
           "FTP postquote runtime exceeded its command budget");
  }

  curl::fuzzer::proto::Scenario http_scenario = scenario;
  http_scenario.set_scheme(curl::fuzzer::proto::SCHEME_HTTP);
  {
    proto_fuzzer::ScenarioRequestData request_data(easy, http_scenario);
    Expect(request_data.stats().ftp_prequote_commands == 0 &&
               request_data.stats().ftp_postquote_commands == 0,
           "HTTP request allocated FTP-only quote lists");
  }
...
void TestGeneratedMimeFileUsesBoundedAnonymousStorage() {
  CURL *easy = curl_easy_init();
  Expect(easy != nullptr, "curl_easy_init failed for MIME file data");

  curl::fuzzer::proto::Scenario scenario;
  auto *file_data =
      scenario.mutable_mime_post()->add_parts()->mutable_file_data();
  ...
  {
    proto_fuzzer::ScenarioRequestData request_data(easy, scenario);
    Expect(request_data.stats().mime_file_bytes ==
               proto_fuzzer::scenario_limits::kMaxGeneratedMimeDataBytes,
           "MIME file source did not expose its bounded generated bytes");
  }
